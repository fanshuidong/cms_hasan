...
(function (factory) {
    ...
    var NAMESPACE, MODAL_ID, STYLE_SETTING, OBJECT_PARAMS, DEFAULT_PREVIEW, objUrl, compare, isIE, handler,
        ...
        tFooter, tActions, tActionDelete, tActionUpload, tActionZoom,tActionSet,tActionDrag, tTagBef, tTagBef1, tTagBef2, tTagAft,
    ...
    previewCache = {
        ...
        init: function (obj) {
            ...
            previewCache.data[id] = {
                ...
                showSet: obj.fileActionSettings.showSet,
                ...
                actions: function (showUpload, showDelete, showZoom,showSet, showDrag, disabled, url, key) {
                    return obj._renderFileActions(showUpload, showDelete, showZoom, showSet,showDrag, disabled, url, key, true);
                }
            };
        },
        fetch: function (id) {
    ...
    defaultFileActionSettings = {
        ...
        //添加属性
        setIcon: '<i class="glyphicon glyphicon-zoom-in"></i>',
    ...
    tActionSet = '<button type="button" class="kv-file-set {setClass}" title="{setTitle}">{setIcon}</button>';
    ...
    FileInput.prototype = {
        ...
        _renderFileFooter: function (caption, size, width, isError) {
            ...
            if (self.isUploadable) {
                out = template.replace(/\{actions}/g, self._renderFileActions(upl, rem, zoom,true, drg, false, false, false))
                ...
                out = template.replace(/\{actions}/g,
                    self._renderFileActions(false, false, zoom,true,drg, false, false, false))
                    .replace(/\{caption}/g, caption).replace(/\{size}/g, size).replace(/\{width}/g, width)
        ...
        _renderFileActions: function (showUpload, showDelete, showZoom,showSet,showDrag, disabled, url, key, isInit) {
            ...
            var self = this,
                ...
                btnDelete = '', btnUpload = '', btnZoom = '',btnSet = '', btnDrag = '', css,
            ...
            if (showSet) {
                btnSet = self._getLayoutTemplate('actionSet')
                    .replace(/\{setClass}/g, config.setClass)
                    .replace(/\{setIcon}/g, config.setIcon)
                    .replace(/\{setTitle}/g, config.setTitle);
            }
            if (showDrag && isInit) {
